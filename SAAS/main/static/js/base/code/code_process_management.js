    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for(let i=0; i<data.length; i++) {
                ...
                html += `        <div class="form-group" style="width:18%;">`;
                html += `            <div class="form-group__title">`;
                html += `                <div class="title">순번</div>`;
                html += `            </div>`;
                html += `            <select name="ordering" class="data-select">`;
                ...
                html += `            </select>`;
                html += `        </div>`;
                html += `        <div class="form-group" style="width:80%;">`;
                html += `            <div class="form-group__title">`;
                html += `                <div class="title">공정명</div>`;
                html += `            </div>`;
                html += `            <input type="text" name="name" class="data-input" placeholder="공정명을 입력해주세요." value="${data[i].name}">`;
                html += `        </div>`;
                html += `    </div>`;
            ...
            for(let i=10; i>data.length; i--) {
                ...
                html += `                <div class="title">공정명</div>`;
                html += `            </div>`;
                html += `            <input type="text" name="name" class="data-input" placeholder="공정명을 입력해주세요.">`;
                html += `        </div>`;
                html += `    </div>`;
                html += `    <div class="code_box__bottom flex-between">`;
                html += `        <div class="toggle_box">`;
...
function createData(self) {
    ...
    /* 필수항목 유효성 검사 */
    const required = ['ordering','name'];
    
    for(let i=0; i<required.length; i++) {
        if(!$code_box.find(`[name='${required[i]}']`).val()) {
            if(required[i] == 'ordering') {
                alert("순번을 선택해주세요.");
            } else if(required[i] == 'name') {
                alert("공정명을 입력해주세요.");
            }
...
function updateData(self) {
    ...
    $.ajax({
        ...
        success:function(data) {
            ...
            } else if(data.result == 'instruction') {
                alert("해당 공정의 생산 진행중인 수주가 존재합니다.\n생산을 모두 완료 후 수정해주시기 바랍니다.");
                return false;
