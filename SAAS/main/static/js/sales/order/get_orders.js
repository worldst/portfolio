...
function getResultData(pageNum, status='', mode=null) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(res.length == 0) {
                $("#list-table tbody").empty().append("<td colspan='9' class='no-data'>현재 등록된 데이터가 없습니다.</td>");

                if(mode === 'search') {
                    $("#list-table tbody").empty().append("<td colspan='9' class='no-data'>조회된 데이터가 없습니다.</td>");
                }
            } else { 
                setTimeout(() => {
                    let html = '';
                    ...
                    $.each(res, function (key, value) {
                        ...
                        html += `    <td><a href="/sales/order/${value.pk}/detail">${value.num}</a></td>`;
                        ...
                        if(value.status == 'WAIT') {
                            html += `    <td><span class="status wait">생산대기</span></td>`;
                        } else if(value.status == 'START') {
                            html += `    <td><span class="status start">생산시작</span></td>`;
                        } else if(value.status == 'PAUSE') {
                            html += `    <td><span class="status pause">일시정지</span></td>`;
                        } else if(value.status == 'CANCEL') {
                            html += `    <td><span class="status cancel">수주취소</span></td>`;
                        } else if(value.status == 'PART') {
                            html += `    <td><span class="status part">부분완료</span></td>`;
                        } else if(value.status == 'COMPLT') {
                            html += `    <td><span class="status complt">수주완료</span></td>`;
                        }
...
function exportExcel(){
    ...
    $.ajax({
        async: false,
        url: '/sales/api/order/excel/download',
        data: obj,
        type: 'GET',
        dataType: 'json',
        beforeSend: function() {
        ...
        success: function (data) {
            ...
            a.href = '/sales/media/sales/order/' + name.toString() + ".xlsx";
...
function workExcel(self) {
    ...
    $.ajax({
        url: '/sales/api/orders/work/excel/download',
        type: 'POST',
        contentType: "application/json",
        data: JSON.stringify(obj),
        beforeSend: function(xhr, settings) {
        ...
        success: function (data) {
            ...
            if(data.sort == 'zip'){
                a.href = '/sales/media/sales/work/' + file_name.toString() + ".zip";
